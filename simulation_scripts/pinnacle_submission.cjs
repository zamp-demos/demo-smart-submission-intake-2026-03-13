...
function runSimulation(port) {
    ...
    return (async () => {
        ...
        await addLog({ step: 7, title: "Financial Analysis", status: "complete",
            description: "FY2022–2024 financial statements analyzed. Total assets: $2.4B. NPL ratio: 0.82% (below 1% threshold). Capital adequacy: Well-capitalized. Earnings trend: stable.",
            timestamp: new Date().toISOString() });
        ...
        await addLog({ step: 9, title: "Appetite Triage Evaluation", status: "complete",
            description: "Risk scored against Chubb FI appetite matrix. Score: 78/100 (Preferred tier). Consent order termination confirmed. Recommended: proceed to quote.",
            timestamp: new Date().toISOString() });
        ...
        await addDetails({ "Premium Indication": "$336,000 (+7.7% over prior year)", "Pace Processing Time": "8 min 22 sec" });
